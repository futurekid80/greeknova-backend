...
def get_iv_analysis(symbol: str = None, date: str = None):
    ...
    # Get all dates we have data for (up to 60 days)
    sixty_ago = (datetime.now(timezone.utc).date() - timedelta(days=60)).isoformat()
    hist_ts_q = supabase.from_("oi_snapshots")\
        .select("timestamp")\
        .eq("symbol", "NIFTY")\
    ...
    # Get one timestamp per day (EOD — last of each day)
    hist_dates: dict = {}
    for r in (hist_ts_q.data or []):
        d = r["timestamp"][:10]
        if d not in hist_dates:
            hist_dates[d] = r["timestamp"]  # first = latest of that day (desc order)
