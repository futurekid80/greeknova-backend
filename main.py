...
# the Sep 2026 63-symbol batch landed in iv_analysis.py/TOP30 but not
# here, so those symbols would have gotten zero CMP/CAS capture despite
# options-chain capture working fine for them). Every existing entry
# followed the same "NSE:" + symbol pattern with no exceptions, so this
# is now generated from the single canonical list instead of duplicated.
